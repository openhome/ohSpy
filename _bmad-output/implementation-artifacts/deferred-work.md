...
- **Pre-existing flaky test — `SubscriptionClientTests.Renew412_Lapses_RenewRefused_NoRetry_NoUnsubscribe`** [`tests/ohSpy.Core.Tests/Events/SubscriptionClientTests.cs:364`] — Observed 1 failure on first Core test run during the 6.3 code review (552 passed, 1 failed); passed on the second run (553/2). Pre-existing timing sensitivity; not caused by Story 6.3. The baseline count is 553/2.
...
## ✅ RESOLVED 2026-06-06 in Story 6.3 — `Program.cs` bootstrap ↔ self-contained contradiction (the install blocker)

**Resolved by Story 6.3 (Option 1 — truly self-contained), architecture Amendment A32.** `Program.cs` no longer calls the framework-dependent `Bootstrap.TryInitialize` / `Bootstrap.Shutdown`; the `0x80670016` `MessageBoxW` failure path + the `DynamicDependency`/`MessageBoxW` plumbing are removed. The csproj `WindowsPackageType=None` + `WindowsAppSDKSelfContained=true` + `SelfContained=true` flags are retained (now real): `Application.Start(_ => new App())` loads the bundled WinAppSDK runtime directly. Headless proof: `dotnet publish src/ohSpy.App -c Release -r win-x64 --self-contained` lays down the WinAppSDK native runtime DLLs (`Microsoft.ui.xaml.dll`, `CoreMessagingXP.dll`, `DWriteCore.dll`, `Microsoft.WindowsAppRuntime.dll`, …) + the .NET runtime (`coreclr.dll`, `clrjit.dll`, `System.Private.CoreLib.dll`, `hostfxr/hostpolicy`) beside `ohSpy.App.exe` (`runtimeconfig.json` carries `includedFrameworks`). The clean-Win11 install/run smoke is now a permanent release-gate step (`docs/DEVELOPMENT.md` + `docs/verification/6.3-…md`) so this can never regress silently. The clean-box launch (no `0x80670016`) is the Project Lead's dry-run gate. Option 2 (framework-dependent + installer-carried `WindowsAppRuntimeInstall-x64.exe`) was the recorded rejected alternative. _Original entry below, retained for history:_
